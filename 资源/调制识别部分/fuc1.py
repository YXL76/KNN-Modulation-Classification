...
def BPSKModulationClassTest():
    sampleDataMat, sampleLabels = file2matrix(
        'data/sample.dat')  # load data setfrom file
    ...
    for snr in SNR:
        testDataMat, testLabels = file2matrix(
            'data/testBPSK-'+str(snr)+'.dat')  # load data setfrom file
        ...
        print("the total correct rate on %d dB SNR is:" %
              (snr), (accuracy[snr]))
        print(correctCount[snr])
    accuracy = collections.OrderedDict(
        sorted(accuracy.items()))  # sort by ascending SNR values
...
def QPSKModulationClassTest():
    ...
    for snr in SNR:
        testDataMat, testLabels = file2matrix(
            'data/testQPSK-'+str(snr)+'.dat')  # load data setfrom file
        numTestVecs = testDataMat.shape[0]
        correctCount[snr] = 0.0
        for i in range(numTestVecs):
            classifierResult[snr] = classify0(
